 ...
 */
export async function convertVideoWithEffect(
  video: KnownVideo,
  ...
  const commands = createCommands(video, convertFormat, effectName);
...
export function createThumbnails(
  ...
  args.push("-vf", `fps=1/${interval},eq=contrast=1000.0,scale=${width}:${height}:flags=bilinear`);
  args.push("-f", `image2`);
